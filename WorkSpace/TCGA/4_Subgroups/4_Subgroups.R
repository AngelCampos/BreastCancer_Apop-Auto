...
# Load data
load("C:/Users/WildFang/Google Drive/Science/BreastCancer_AA Project/WorkSpace/TCGA/1_Deregulation_Analysis/fPDS.Rdata")
...
load("C:/Users/WildFang/Google Drive/Science/BreastCancer_AA Project/WorkSpace/TCGA/3_PAM50/genefu_PAM50.Rdata")
normals <- as.logical(onlyFemNormals); rm(onlyFemNormals)
...
png("Subgroups_TCGA.png", # Name of png file
...
heatmap.2(fPDS,
          main = "PDS-Subgroups-TCGA",   # heat map title
...
legend("topright",      # location of the legend on the heatmap plot
       legend = c("SG1", "Healthy", "SG2", "SG3", "SG4"), # category labels
       col = ccol,  # color key
       lty= 1,          # line style
...
for (i in c("Basal", "LumA", "LumB", "Her2", "Normal")){
  g <- as.logical(genefuPAM50 == i) & as.logical(subg == 1)
  y <- sum(g)
  y0 <- cbind(y0, y) 
}
y1 <- NULL
for (i in c("Basal", "LumA", "LumB", "Her2", "Normal")){
  g <- as.logical(genefuPAM50 == i) & as.logical(subg == 2)
  y <- sum(g)
  y1 <- cbind(y1, y) 
}
y2 <- NULL
for (i in c("Basal", "LumA", "LumB", "Her2", "Normal")){
  g <- as.logical(genefuPAM50 == i) & as.logical(subg == 3)
  y <- sum(g)
  y2 <- cbind(y2, y) 
}
y3 <- NULL
for (i in c("Basal", "LumA", "LumB", "Her2", "Normal")){
  g <- as.logical(genefuPAM50 == i) & as.logical(subg == 4)
  y <- sum(g)
  y3 <- cbind(y3, y) 
}
y4 <- NULL
for (i in c("Basal", "LumA", "LumB", "Her2", "Normal")){
  g <- as.logical(genefuPAM50 == i) & as.logical(subg == 5)
  y <- sum(g)
  y4 <- cbind(y4, y) 
}
...
rownames(pv_table) <- c("SG1","Healthy","SG2","SG3","SG4")

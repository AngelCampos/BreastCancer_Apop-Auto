################################################################################
# Deregulation Analysis Pathifier - METABRIC
...
save(PDS, file = "METABRIC_PDS.Rdata") # Save PDS object
...
save(ZscorePDS, file = "ZscorePDS_METABRIC.Rdata")
...
png("PDS_Heatmap_METABRIC.png", # Name of png file
...
heatmap.2(fPDS,
          main = "PDS-Heatmap-METABRIC",   # heat map title
...
png("PDS-Zscore_Heatmap_METABRIC.png", # Name of png file
    width = 6 * 500,      # Easier scaling 6*500 = 3000 pixels
    height = 6 * 400,     # 6 x 400 = 2400 px
    units = "px",         # px (Pixels = default), in (inches), cm or mm
    res = 300,            # 300 pixels per inch
...
heatmap.2(ZscorePDS,
          main = "PDS-Zscore-Heatmap-METABRIC",   # heat map title
          density.info = "none",  # turns off density plot inside color legend
          trace = "none",         # turns off trace lines inside the heat map
          margins = c(10,21),     # widens margins around plot
          col = my_palette,       # use on color palette defined earlier
...
dev.off()               # close the PNG device
save.image("CheckPoint_1.Rdata") # Check Point! ###########################